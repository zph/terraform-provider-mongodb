...
* `insecure_skip_verify` - (Optional) Skip TLS certificate verification (chain and hostname). Applies whether or not `certificate` is set; requires `ssl = true` or a `certificate` to take effect. Default: `false`.
