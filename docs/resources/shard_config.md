...
Before any member is added, the resource connects directly to each new host and asks it for its own replica set status. A host that is already an active member of this set under another name, for example `mongo2.internal:27017` for a member the set knows as `mongo2:27017`, that belongs to a different set, or that is not running with `--replSet`, is refused before anything is sent, and the error names the host the set knows it by. A fresh host, or one removed from the set earlier, is added. A host the Terraform runner cannot reach or authenticate to is added anyway, with a log line saying it could not be inspected; the wait below still catches a mistyped host. The check is skipped when `host_override` is set, since the member hostnames are then not reachable from the runner.
