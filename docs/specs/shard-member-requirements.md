...
SHALL treat the block as a member to add (SHARD-012 through SHARD-017) rather
...
the reconfig that applies the settings and the merged members. MongoDB 4.4
rejects a non-force reconfig that adds more than one voting member; one member
per reconfig is correct on every supported version.
...
apply every per-member field from the block (priority, votes, hidden,
arbiter_only, build_indexes, tags).

**SHARD-016** (Event Driven): WHEN a reconfig that adds a member succeeds, the
resource SHALL poll `replSetGetStatus` until the primary reports the new
member with `health: 1` in any state, or until `init_timeout_secs` elapses.
Initial sync is not waited for: it can take hours on a data-bearing set, and a
member that is still syncing surfaces through the oplog read-back (OPLOG-016).

**SHARD-017** (Unwanted Behaviour): IF the new member does not report
...
also fails, the error SHALL report both failures.
...
resource SHALL record the resource ID and the settings in state before adding
members, so a failed add cannot leave an applied reconfig unrecorded. A
following apply partitions against the live configuration again and adds only
what is still missing.

## Initialization

The initialization flow hands over to this reconciliation after
`replSetInitiate` of the first member (INIT-010); SHARD-012 through SHARD-021
apply unchanged.
